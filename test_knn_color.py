...
def test(df):
    ...
    result = pd.DataFrame(columns=['knn_color1','knn_color3','knn_color5','knn_color7','knn_color9'])
    index = 0
    for t in range(30):
        print("test:"+str(t))
        ...
        for j in range(10):
            ...
            knn_shape_correct = knn.knn_dataframes(test_color, train_color, 1)
            knn_color_correct = knn.knn_dataframes(test_color, train_color, 3)
            baysian_shape_correct = knn.knn_dataframes(test_color, train_color, 5)
            baysian_color_correct = knn.knn_dataframes(test_color, train_color, 7)
            majority_correct = knn.knn_dataframes(test_color, train_color, 9)
        ...
        df = df.sample(frac=1)
    result.mean().to_csv("knn_color.csv")
test(df)